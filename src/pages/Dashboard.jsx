...
const Dashboard = () => {
  ...
  const generateFileName = (extension) => {
    ...
    return `voters_export_${hours}_${minutes}_${day}_${month}_${year}.${extension}`;
  };
  ...
  const handleExport = async (format) => {
    ...
    try {
      if (format === "json") {
        ...
        const csvData = Papa.unparse(voters, {
          header: true,
          columns: [
            "voterId",
            "fullName",
      ...
      toast.success(`Voters exported successfully as ${format.toUpperCase()}!`);
    } catch (err) {
      console.error(`Error exporting voters as ${format}:`, err);
      toast.error(`Failed to export voters as ${format.toUpperCase()}`);
    } finally {
  ...
  const handleFileSelect = (event) => {
    ...
    if (
      ...
      setMessage({
        type: "error",
        text: "Please select a valid JSON or CSV file",
      });
    }
  };
      ...
      if (uploadedFile.type === "application/json") {
        ...
        const result = Papa.parse(text, {
          ...
          transform: (value, field) => {
            ...
            if (
              field === "assemblyConstituencyNumber" ||
              field === "lokSabhaConstituencyNumber"
            ) {
        ...
        if (result.errors.length > 0) {
          throw new Error("Invalid CSV format: " + result.errors[0].message);
        }
        parsedVoters = result.data;
      }

      if (!Array.isArray(parsedVoters)) {
        throw new Error("File must contain an array of voter objects");
      }
  ...
  const handleDrop = (e) => {
    ...
    if (
      ...
      setMessage({
        type: "error",
        text: "Please drop a valid JSON or CSV file",
      });
    }
  };
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              Download all voter data as JSON or CSV file
            </p>
          </div>
          <div className="flex space-x-4">
            ...
            <button
              ...
              disabled={exportLoading[exportFormat] || voters.length === 0}
            ...
            >
              ...
              Export Voters ({exportFormat.toUpperCase()})
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="mb-4">
          ...
          <p className="text-gray-600">
            Upload JSON or CSV file to bulk import voter data
          </p>
        </div>
        ...
        >
          ...
          <div className="space-y-2">
            <p className="text-lg font-medium text-gray-900">
              Drop your JSON or CSV file here, or{" "}
              <button
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <p className="text-sm text-blue-700">
            JSON: Array of objects with fields: voterId, fullName, firstName,
            lastName, relativeName, houseNo, addressLine1, addressLine2, gender,
            age, mobileNumber, caste, etc.
          </p>
          <p className="text-sm text-blue-700 mt-1">
            CSV: Header row with fields: Voter ID, Full Name, First Name, Last
            Name, Relative Name, House Number, Address Line 1, Address Line 2,
